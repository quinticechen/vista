...
export const HeadingRenderer: React.FC<BlockRendererProps> = ({ block, index, listPath }) => {
  ...
  switch (type) {
    ...
    case "heading_3":
      return (
        <h3 key={`h3-${listPath}-${index}`} className="text-xl font-bold mt-4 mb-2">
          {emoji && <span className="mr-2">{renderIcon({emoji})}</span>}
          {renderTextWithLineBreaks(block)}
        </h3>
      );
...
export const QuoteRenderer: React.FC<BlockRendererProps> = ({ block, index, listPath }) => {
  return (
    <blockquote key={`quote-${listPath}-${index}`} className="border-l-4 border-muted pl-4 py-1 my-2" style={{ lineHeight: '1.6' }}>
      {renderTextWithLineBreaks(block)}
    </blockquote>
  );
};
...
export const TodoRenderer: React.FC<BlockRendererProps> = ({ block, index, listPath }) => {
  ...
  return (
    <div key={`todo-${listPath}-${index}`} className="flex items-start gap-2 my-2" style={{ lineHeight: '1.6' }}>
      <input 
        type="checkbox" 
        checked={checked} 
        readOnly 
...
export const CalloutRenderer: React.FC<BlockRendererProps> = ({ block, index, listPath, renderNested }) => {
  ...
  return (
    <div key={`callout-${listPath}-${index}`} className="bg-muted p-4 rounded-md my-2 flex gap-3 items-start" style={{ lineHeight: '1.6' }}>
      {(icon || emoji) && (
...
export const CodeRenderer: React.FC<BlockRendererProps> = ({ block, index, listPath }) => {
  ...
  return (
    <pre key={`code-${listPath}-${index}`} className="bg-muted p-4 rounded-md my-2 overflow-x-auto" style={{ lineHeight: '1.6' }}>
...
export const DefaultBlockRenderer: React.FC<BlockRendererProps> = ({ block, index, listPath }) => {
  if (block.text) {
    return (
      <div key={`default-${listPath}-${index}`} className="my-2" style={{ lineHeight: '1.6' }}>
        {renderTextWithLineBreaks(block)}
      </div>
    );
  }
...
export const DividerRenderer: React.FC<{ index: number, listPath: string }> = ({ index, listPath }) => {
  return <hr key={`divider-${listPath}-${index}`} className="my-4 border-t border-gray-400" />;
};
